...
def get_supported_tts_providers() -> List[str]:
    return [TTS_AZURE, TTS_OPENAI, TTS_JLSPEECH]
...
def get_tts_provider(config) -> BaseTTSProvider:
    ...
    elif config.tts == TTS_JLSPEECH:
        from audiobook_generator.tts_providers.jlspeech_tts_provider import JLSpeechTTSProvider
        return JLSpeechTTSProvider(config)
    else:
